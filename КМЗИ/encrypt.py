...
def Hiding(fname, message):
    ...
    if len(binary) > (width * height):
        raise Exception ("Size of image too small - choose a larger image")
    if mode in ('RGBA'):
        ...
        img.save('test_hiding.png', 'png')
        

		
def main(data, key_vig, key_aes, imageFile):
    cd = 'c'		
    do = {'c': lambda x, y: x + y, 'd': lambda x, y: x - y}
    code = vigenere(data, key_vig, do[cd])	
    ...
    for byte in code:
        temp.append(byte)
        if len(temp) == 16:
            crypted_part = encrypt(temp, key_aes)
    ...
    #crypted_data.append(temp)
        if 0 < len(temp) < 16:
            ...
            for i in range(empty_spaces - 1):
                temp.append(0)
            temp.append(1)
            crypt_part = encrypt(temp, key_aes)
            crypted_data.extend(crypt_part)
            
    print('AES-vipolnen')			
    try:
        Hiding(imageFile, bytearray(crypted_data))
    except Exception as err:
        exit("Error: {0}".format(err))	
    with open('test_hiding.png', 'rb') as f:
        res = f.read()
    print('LSB-vipolnen')
